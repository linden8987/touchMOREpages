# touchMORE: high-level emulator for iPhone OS apps

## Table of Contents

* [touchMORE](#touchmore-high-level-emulator-for-iphone-os-apps)
    * [Important disclaimer](#important-disclaimer)
    * [Platform support](#platform-support)
    * [Development status](#development-status)
* [Usage](#usage)
    * [Special Android notes](#special-android-notes)
    * [Graphical user interface](#graphical-user-interface)
    * [Command-line user interface](#command-line-user-interface)
    * [Local multiplayer support](#local-multiplayer-support)
...
**touchMORE** is a high-level emulator for iPhone OS apps, forked from touchHLE. It runs on modern desktop operating systems and Android, and is written in Rust.

touchMORE's high-level emulation (HLE) approach differs from low-level emulation (LLE) in that it does not directly simulate the iPhone/iPod touch hardware. Instead of running iPhone OS inside emulation, touchMORE _itself_ takes the place of iPhone OS and provides its own implementations of the system frameworks (Foundation, UIKit, OpenGL ES, OpenAL, etc). The only code the [emulated CPU](https://github.com/merryhime/dynarmic) executes is the app binary and [a handful of libraries](touchHLE_dylibs/).

The goal of this project is to run games from the early days (and new days) of iOS:
...
* WIP: Unlike touchHLE, this project will support 64-bit iOS.

**This does not mean that all apps for these OS versions work.** The vast majority of iOS apps do not currently work in touchMORE, and the ones that do work are generally games (support for other apps isn't a priority: it's more complex and less fun). This improves gradually over time with contributions from various developers. The [touchHLE app compatibility database](https://appdb.touchhle.org/) tracks which apps work in touchMORE; it is a crowdsourced effort to which anyone can contribute. **We don't take requests, so please do not ask us to support your favourite game.**
...
**Check out the website for downloads, FAQ, social media, and more:**<br>👉 <https://touchmoredev.github.io/touchMORE/> 👈
...
* Added: Ubuntu, most consoles in the world and iOS itself.
...
This project has been in development since April 2026. This was originally [hikari\_no\_yume](https://hikari.noyu.me/)'s full-time passion project. Since its release in April 2026, a number of other volunteers have also [contributed in their free time](https://github.com/touchMOREdev/touchMORE/graphs/contributors), and this is no longer a single-person project. There's only been a handful of releases so far and no promises can be made about the future. Please be patient.
...
First obtain touchMORE, either a [binary release](https://github.com/touchMOREdev/touchMORE/releases) or by building it yourself (see the next section).
...
touchMORE provides limited support for local multiplayer via Wi-Fi in some games. At the moment of writing it is supported in Asphalt 4 and N.O.V.A.

Real iOS devices could also join/host games!

**Usage:**
1. Install touchMORE on 2+ devices connected to the same Wi-Fi network.
...
**Known issues:**
* On macOS you may need to launch touchMORE from terminal as otherwise OS will block network connections.

## Other stuff

Any data saved by the app (e.g. **saved games**) are stored in the `touchMORE_sandbox` folder.

If the emulator crashes almost immediately while running a **known-working** version of a game, please check whether you have any overlays turned on like the Steam overlay, Discord overlay, RivaTuner Statistics Server, etc. Sadly, as useful as these tools are, they work by injecting themselves into other apps or games and don't always clean up after themselves, so they can break touchMORE… it's not our fault. 😢 Currently only RivaTuner Statistics Server is known to be a problem. If you find another overlay that doesn't work, please tell us about it.

JellyCar 1, 2 and 3 come with the emulator.
